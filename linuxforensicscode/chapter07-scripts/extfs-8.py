...
"""
Class ExtentIndex.  Represents a middle
or index node in an extent tree.  Accepts
a 12-byte packed string containing the index.
...
"""
Class Extent.  Represents a leaf node
or extent in an extent tree.  Accepts
a 12-byte packed string containing the extent.
...
"""
def getExtAttrsFromBlock(imageFilename, offset, blockNo, blockSize=4096):
  data = getDataBlock(imageFilename, offset, blockNo, blockSize)
  return getExtAttrsHelper(False, imageFilename, offset, data, blockSize)
...
where imageFilename is a raw ext2/ext3/ext4 image, offset is
the offset in 512 byte sectors to the start of the filesystem,
data is the packed string holding the extended attributes, and
blocksize is the filesystem block size (default=4k).
"""
    ...
    for k, v in sorted(self.__dict__.iteritems()) :
      if k == 'extents' and self.extents:
        ...
        for i in range(1, v[0].entries + 1):
          v[i].prettyPrint()
      elif k == 'changeTime' or k == 'modifyTime' or k == 'accessTime' \
        or k == 'createTime':
        print k+":", time.asctime(v)
      elif k == 'deleteTime':
